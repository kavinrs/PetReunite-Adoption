...
export async function fetchNearbyUsers(radiusKm?: number, customLat?: number, customLon?: number): Promise<ApiResult> {
  let params = radiusKm ? `?radius=${radiusKm}` : '';
  
  
  if (customLat !== undefined && customLon !== undefined) {
    params += (params ? '&' : '?') + `lat=${customLat}&lon=${customLon}`;
  }
  
  const url = `${API_BASE}/users/nearby/${params}`;
  try {
    const resp = await fetchWithAuth(url, {
      method: "GET",
    });
    ...
    const message = extractErrorMessage(data) ?? "Failed to fetch nearby users";
    return { ok: false, status: resp.status, error: message, data };
  } catch (error) {
...
export async function sendDirectChatRequest(receiverId: number, reason: string): Promise<ApiResult> {
  const url = `${API_BASE}/pets/direct-chat-requests/create/`;
  try {
    const resp = await fetchWithAuth(url, {
      method: "POST",
    ...
    const message = extractErrorMessage(data) ?? "Failed to send chat request";
    return { ok: false, status: resp.status, error: message, data };
  } catch (error) {
...
export async function fetchDirectChatRequests(type: 'sent' | 'received' = 'received'): Promise<ApiResult> {
  const url = `${API_BASE}/pets/direct-chat-requests/?type=${type}`;
  try {
    const resp = await fetchWithAuth(url, {
      method: "GET",
    });
    ...
    const message = extractErrorMessage(data) ?? "Failed to fetch chat requests";
    return { ok: false, status: resp.status, error: message, data };
  } catch (error) {
...
export async function acceptDirectChatRequest(requestId: number): Promise<ApiResult> {
  const url = `${API_BASE}/pets/direct-chat-requests/${requestId}/accept/`;
  try {
    const resp = await fetchWithAuth(url, {
      method: "POST",
    });
    ...
    const message = extractErrorMessage(data) ?? "Failed to accept chat request";
    return { ok: false, status: resp.status, error: message, data };
  } catch (error) {
...
export async function rejectDirectChatRequest(requestId: number): Promise<ApiResult> {
  const url = `${API_BASE}/pets/direct-chat-requests/${requestId}/reject/`;
  try {
    const resp = await fetchWithAuth(url, {
      method: "POST",
    });
    ...
    const message = extractErrorMessage(data) ?? "Failed to reject chat request";
    return { ok: false, status: resp.status, error: message, data };
  } catch (error) {
