...
class ConversationCreateSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if pet_unique_id:
            ...
            data["pet_unique_id"] = pet_unique_id                                   
...
class ChatroomSerializer(serializers.ModelSerializer):
    ...
    def get_participants(self, obj):
        """Return list of participants with their user details"""
        participants = obj.participants.filter(is_active=True).select_related('user', 'user__profile')
        result = []
        for p in participants:
            ...
            if hasattr(p.user, 'profile'):
                ...
                user_data['full_name'] = profile.full_name or p.user.username
                user_data['user_unique_id'] = profile.user_unique_id
...
class DirectChatRequestSerializer(serializers.ModelSerializer):
    """Serializer for direct chat requests between nearby users"""
    sender = UserSummarySerializer(read_only=True)
