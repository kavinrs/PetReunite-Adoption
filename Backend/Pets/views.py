...
class DirectChatRequestCreateView(APIView):
    """Create a direct chat request to another user"""
    permission_classes = [permissions.IsAuthenticated]
    ...
    def post(self, request):
        ...
        if len(reason) > 500:
            return Response(
                {'detail': 'Reason must be 500 characters or less'},
                status=status.HTTP_400_BAD_REQUEST
            )
        ...
        if receiver == request.user:
            return Response(
                {'detail': 'Cannot send chat request to yourself'},
                status=status.HTTP_400_BAD_REQUEST
            )
        ...
        chat_request = DirectChatRequest.objects.create(
            sender=request.user,
            receiver=receiver,
            reason=reason
        )
        ...
        Notification.objects.create(
            recipient=receiver,
            notification_type='direct_chat_request',
            title='New Chat Request',
            message=f'{request.user.username} wants to chat with you',
            from_user=request.user
        )
        
        serializer = DirectChatRequestSerializer(chat_request)
        return Response(serializer.data, status=status.HTTP_201_CREATED)


class DirectChatRequestListView(generics.ListAPIView):
    """List direct chat requests (sent and received)"""
    permission_classes = [permissions.IsAuthenticated]
    serializer_class = DirectChatRequestSerializer
    ...
    def get_queryset(self):
        ...
        if request_type == 'sent':
            return DirectChatRequest.objects.filter(sender=user).order_by('-created_at')
        else:
            return DirectChatRequest.objects.filter(receiver=user).order_by('-created_at')


class DirectChatRequestAcceptView(APIView):
    """Accept a direct chat request"""
    permission_classes = [permissions.IsAuthenticated]
    
    def post(self, request, request_id):
        try:
            chat_request = DirectChatRequest.objects.get(
                id=request_id,
            ...
            try:
                ...
                sender_chatrooms = Chatroom.objects.filter(participants__user=chat_request.sender)
                receiver_chatrooms = Chatroom.objects.filter(participants__user=request.user)
                existing_chatrooms = sender_chatrooms.filter(id__in=receiver_chatrooms.values_list('id', flat=True))
                direct_chatrooms = existing_chatrooms.filter(pet_id__isnull=True, purpose='')
                chatroom = direct_chatrooms.first()
                ...
                if chatroom:
                    ...
                    return Response(
                        {'detail': 'Chat request already accepted but chatroom not found'},
                        status=status.HTTP_404_NOT_FOUND
                    )
            except DirectChatRequest.DoesNotExist:
                return Response(
                    {'detail': 'Chat request not found or already processed'},
                    status=status.HTTP_404_NOT_FOUND
                )
        ...
        try:
            ...
            if not chatroom:
                ...
                conversation = Conversation.objects.create(
                    ...
                    pet_name='',
                    pet_kind='',
                    reason_for_chat=chat_request.reason,
                    status='active'
                )
                
                chatroom = Chatroom.objects.create(
                    name=f"Direct Chat: {request.user.username} & {chat_request.sender.username}",
                    conversation=conversation,
                    created_by=request.user,
                    purpose=''                                       
                )
            ...
            Notification.objects.create(
                recipient=chat_request.sender,
                notification_type='direct_chat_accepted',
            ...
            print(f"Error accepting direct chat request: {str(e)}")
            print(error_details)
            return Response(
                {'detail': f'Failed to accept chat request: {str(e)}'},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )


class DirectChatRequestRejectView(APIView):
    """Reject a direct chat request"""
    permission_classes = [permissions.IsAuthenticated]
    ...
    def post(self, request, request_id):
        ...
        Notification.objects.create(
            recipient=chat_request.sender,
            notification_type='direct_chat_rejected',
